...
const EMPTY: FilterState = {
  q: '', source: '', bucket: '', hardGate: '', intent: '', children: '', affection: '',
  libido: '', communication: '', verification: '', state: '', risk: '', ageMin: '', ageMax: '',
};
...
type Tab = 'inspector' | 'audit' | 'scores' | 'comparison' | 'tuning';
    ...
    <div className="grid gap-6">
      ...
      <section className="card p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <p className="label">Synthetic profile browser</p>
        ...
        <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <Select label="Recommendation bucket" value={f.bucket} onChange={set('bucket')} options={filters.buckets.map((b) => ({ value: b.value, label: b.label }))} />
          <Select label="Hard gate" value={f.hardGate} onChange={set('hardGate')} options={[{ value: 'pass', label: 'Pass' }, { value: 'fail', label: 'Fail' }]} />
        ...
        <p className="mt-3 text-sm text-stone-500">{filtered.length.toLocaleString()} of {rows.length.toLocaleString()} synthetic profiles match{shown.length < filtered.length ? ` · showing first ${shown.length}` : ''}.</p>
      </section>

      <div className="grid gap-6 xl:grid-cols-[minmax(320px,420px)_1fr]">
        {/* Browser list */}
        <section className="grid max-h-[720px] gap-2 overflow-y-auto pr-1">
          {shown.map((r) => (
            <button
              key={r.id}
              onClick={() => select(r.id)}
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className={`pill ${bucketTone[r.queueStatus]}`}>{r.queueLabel}</span>
              </div>
              <p className="mt-1 text-xs text-stone-600">{r.age} · {r.region} · {r.relationshipIntent}</p>
              <div className="mt-2 flex flex-wrap gap-1.5">
                <span className="pill">{r.sourceLabel}</span>
                <span className="pill">{r.hardGatePass ? 'gate: pass' : 'gate: fail'}</span>
                <span className="pill">conf {r.recommendationConfidence}%</span>
              </div>
        ...
        {/* Detail */}
        <section>
          {!selectedId && <div className="card p-8 text-stone-500">Select a synthetic profile to inspect how the AI/matching system processed it.</div>}
          {selectedId && loading && <div className="card p-8 text-stone-500">Loading profile audit…</div>}
            ...
            <div className="grid gap-4">
              <div className="card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="label">{detail.normalized.sourceLabel} · {detail.row.id}</p>
                    <h2 className="mt-1 text-2xl font-black tracking-[-.03em]">{detail.row.name}</h2>
                    <p className="text-sm text-stone-600">{detail.row.age} · {detail.row.region} · {detail.inspector.relationshipIntent}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className={`pill ${bucketTone[detail.row.queueStatus]}`}>{detail.row.queueLabel}</span>
                    <span className="text-xs text-stone-500">{detail.row.hardGatePass ? 'hard gate: pass' : 'hard gate: fail'}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {([
                  ['comparison', 'Comparison debugger'],
                  ['scores', 'Score breakdown'],
                  ['audit', 'AI process / audit'],
              ...
              {tab === 'comparison' && (
                <div className="card grid gap-4 p-5">
                  <div className="flex flex-wrap items-center gap-3">
                    <p className="label">Compared against seed: {seed.name} · {seed.region}</p>
                    <span className="pill">{seed.relationshipIntent}</span>
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field label="Final recommendation confidence" value={`${detail.comparison.recommendationConfidence}%`} />
                    <Field label="Hard gate" value={detail.comparison.hardGatePass ? 'PASS' : 'FAIL'} />
                  </div>
              ...
              {tab === 'audit' && (
                <div className="card grid gap-4 p-5">
                  <p className="label">AI process / audit · how this profile was processed</p>
                  <Field label="Recommendation bucket assigned" value={detail.audit.bucketAssigned} />
              ...
              {tab === 'inspector' && (
                <div className="grid gap-4">
                  <div className="card grid gap-3 p-5">
                    <p className="label">Profile inspector · normalized record</p>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      ...
                      <Field label="Relationship intent" value={detail.inspector.relationshipIntent} />
                  ...
                  <div className="card grid gap-3 p-5">
                    ...
                    <p className="text-sm text-stone-600">{detail.sourceNote}</p>
                    {detail.sourceData ? (
                      <pre className="max-h-[420px] overflow-auto rounded-xl bg-[#11201d] p-4 text-xs leading-relaxed text-emerald-50">{JSON.stringify(detail.sourceData, null, 2)}</pre>
                    ) : (
                      <pre className="max-h-[420px] overflow-auto rounded-xl bg-[#11201d] p-4 text-xs leading-relaxed text-emerald-50">{JSON.stringify(detail.normalized, null, 2)}</pre>
                    )}
                  </div>
                </div>
              )}
